export default function WinnersTable({ prizes, onChange }) {
  const items = [
    prizes.slice(0, prizes.length / 2),
    prizes.slice(prizes.length / 2, prizes.length),
  ];
  return (
    <div className="row">
      {items.map((prizeColumn) => (
        <div className="col col-sm-6">
          <table className="table text-left prize-table">
            ...
            <tbody>
              {prizeColumn.map(({ name, amount, winner, id }) => {
                const completedClassName = winner
                  ? 'bg-primary text-white'
                  : '';
                return (
                  <tr>
                    <td className={completedClassName}>{name}</td>
                    <td className={`text-right ${completedClassName}`}>
                      ₹ {amount}
                    </td>
