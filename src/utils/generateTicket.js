...
export default function generateTicket() {
	...
	const ticket = positionsToFill.map(
		rowPositionsToFill => positionsInARow.map((p) => rowPositionsToFill.includes(p) ? 'x' : ''),
	);
	
	positionsInARow.forEach(colIndex => {
		const numbersForColumn = shuffle(getNumbersForColumn(colIndex));
		const filledColumn = column(ticket, colIndex).map(val => val === 'x' ? numbersForColumn.pop() : val);
